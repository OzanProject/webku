...
<div x-data="{ showImportModal: false }" class="p-md md:p-lg lg:p-xl mt-16 max-w-container-max mx-auto w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-md mb-lg">
        <div>
            <h2 class="text-headline-md font-headline-md font-bold text-on-surface">Manajemen Layanan</h2>
            <p class="text-body-md font-body-md text-on-surface-variant">Kelola daftar layanan utama yang Anda tawarkan.</p>
        </div>
        <div class="flex items-center gap-sm w-full sm:w-auto">
            ...
            <a href="{{ route('admin.services.create') }}" class="inline-flex items-center gap-sm bg-primary text-on-primary px-md py-sm rounded-lg hover:bg-primary/90 transition-colors shadow-sm font-label-md whitespace-nowrap">
    ...
    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden mb-xl">
        <div class="p-md border-b border-outline-variant flex flex-col xl:flex-row justify-between xl:items-center bg-surface-bright gap-md">
            <h3 class="text-body-lg font-body-lg font-bold text-on-surface whitespace-nowrap">Daftar Layanan</h3>
            
            <form action="{{ route('admin.services.index') }}" method="GET" class="w-full sm:w-auto flex flex-col sm:flex-row gap-sm items-center">
                <!-- Data Per Page -->
                ...
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari layanan..." class="w-full pl-xl pr-sm py-sm rounded-lg border border-outline-variant/50 focus:border-primary focus:ring-1 focus:ring-primary bg-surface text-body-md outline-none transition-colors">
                    
                    @if(request('search'))
                    <a href="{{ route('admin.services.index') }}" class="absolute right-sm top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-error transition-colors flex items-center justify-center">
            ...
            <table class="w-full text-left border-collapse min-w-[800px]">
                <thead>
                    <tr class="bg-surface-container-lowest text-on-surface-variant text-label-sm font-label-sm uppercase tracking-wider">
                        ...
                        <th class="p-md font-medium border-b border-outline-variant">Layanan</th>
                        <th class="p-md font-medium border-b border-outline-variant">Deskripsi Singkat</th>
                        <th class="p-md font-medium border-b border-outline-variant w-24">Urutan</th>
                ...
                <tbody class="text-body-md font-body-md divide-y divide-outline-variant">
                    ...
                    <tr class="hover:bg-surface-container-low transition-colors group">
                        <td class="p-md text-on-surface-variant">{{ $services->firstItem() + $index }}</td>
                        <td class="p-md">
                            <div class="flex items-center gap-md">
                                <div class="w-12 h-12 rounded-lg bg-surface-container-high flex items-center justify-center text-primary border border-outline-variant/30 shrink-0">
                                    <span class="material-symbols-outlined text-[24px]">{{ $service->icon }}</span>
                                </div>
                                <h4 class="font-bold text-on-surface whitespace-nowrap">{{ $service->title }}</h4>
                            </div>
                        </td>
                        <td class="p-md text-on-surface-variant truncate max-w-[300px]">
                            {{ $service->description }}
                        </td>
                        ...
                        <td class="p-md text-right">
                            <div class="flex justify-end gap-sm">
                                <a href="{{ route('admin.services.edit', $service->id) }}" class="p-sm text-on-surface-variant hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title="Edit">
                    ...
                    <tr>
                        <td colspan="6" class="p-xl text-center">
                            <div class="flex flex-col items-center justify-center text-on-surface-variant">
                                <span class="material-symbols-outlined text-[48px] mb-md opacity-50">design_services</span>
                                <p class="text-body-lg font-bold mb-xs">Belum Ada Layanan</p>
                                <p class="text-body-md opacity-80 mb-md">Anda belum menambahkan layanan apapun.</p>
                                <a href="{{ route('admin.services.create') }}" class="text-primary hover:underline font-label-md">Tambah Layanan Pertama</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div x-show="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" style="display: none;" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95">
        <div @click.outside="showImportModal = false" class="bg-surface w-full max-w-md mx-4 rounded-2xl shadow-xl overflow-hidden">
            <div class="px-lg py-md border-b border-outline-variant flex items-center justify-between bg-surface-container-lowest">
                <h3 class="text-headline-sm font-headline-sm font-bold text-on-surface">Import Data Layanan</h3>
            ...
            <div class="p-lg">
                <p class="text-body-md text-on-surface-variant mb-md">Gunakan file template Excel untuk mengimpor data layanan secara massal.</p>
                
                <a href="{{ route('admin.services.template') }}" class="inline-flex items-center gap-sm px-md py-sm bg-surface-container-high hover:bg-surface-container-highest border border-outline-variant rounded-lg text-on-surface font-label-md transition-colors w-full justify-center mb-lg">
                ...
                <form action="{{ route('admin.services.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-lg">
                        ...
                        <div class="border-2 border-dashed border-outline-variant rounded-xl p-xl text-center hover:border-primary hover:bg-primary/5 transition-colors cursor-pointer relative">
                            <input type="file" name="file" accept=".xlsx,.csv" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" onchange="document.getElementById('fileNameService').innerText = this.files[0] ? this.files[0].name : 'Belum ada file dipilih'">
                            ...
                            <p id="fileNameService" class="text-label-sm text-primary mt-2">Belum ada file dipilih (.xlsx, .csv)</p>
                        </div>
                    </div>
