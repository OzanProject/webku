...
<div class="p-md md:p-lg lg:p-xl mt-16 max-w-container-max mx-auto w-full">
    <div class="flex items-center gap-sm mb-lg">
        ...
        <h2 class="text-headline-md font-headline-md font-bold text-on-surface">Edit Kategori</h2>
    </div>

    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="p-lg">
            @csrf
            @method('PUT')
            
            <div class="mb-md">
                <label class="block text-label-md font-bold text-on-surface mb-xs">Nama Kategori <span class="text-error">*</span></label>
                <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="w-full px-md py-sm rounded-lg border border-outline-variant/50 focus:border-primary focus:ring-1 focus:ring-primary bg-surface text-body-md outline-none transition-colors">
            ...
            <div class="mb-md">
                ...
                <textarea name="description" rows="3" class="w-full px-md py-sm rounded-lg border border-outline-variant/50 focus:border-primary focus:ring-1 focus:ring-primary bg-surface text-body-md outline-none transition-colors">{{ old('description', $category->description) }}</textarea>
            ...
            <div class="mb-lg">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded border-outline-variant/50 text-primary focus:ring-primary">
