...
<div class="p-md md:p-lg lg:p-xl mt-16 max-w-container-max mx-auto w-full">
    <div class="flex items-center gap-sm mb-lg">
        ...
        <h2 class="text-headline-md font-headline-md font-bold text-on-surface">Tambah Keunggulan</h2>
    </div>

    <div class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden max-w-3xl">
        <form action="{{ route('admin.features.store') }}" method="POST" class="p-lg">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-md mb-md">
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon', 'verified') }}" required class="w-full px-md py-sm rounded-lg border border-outline-variant/50 focus:border-primary focus:ring-1 focus:ring-primary bg-surface text-body-md outline-none transition-colors">
                    <p class="text-label-sm text-on-surface-variant mt-1">Gunakan teks <a href="https://fonts.google.com/icons?icon.set=Material+Symbols" target="_blank" class="text-primary hover:underline">Material Symbols</a>. Cth: <code>verified</code>, <code>speed</code>, <code>support_agent</code></p>
